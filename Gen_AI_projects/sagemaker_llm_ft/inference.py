...
def input_fn(request_body, request_content_type):
    if request_content_type == 'application/json':
        ...
        raise ValueError(f'Expected type: application/json ; Actual type: {request_content_type}')
...
def predict_fn(input_data, model):
    ...
    return {'predicted_label': predicted_label, 'proba': probabilities.tolist()}

def output_fn(prediction, accept):
    if accept == 'application/json':
        return json.dumps(prediction), accept
    else:
        raise ValueError(f'Unexpected value for accept: {accept}')